...
class ReviewOpportunitiesCubit extends BaseCubit{
  ...
  fetchPendingConfirmationOpportunities() async {
    try{
      emit(LoadingState());
      final response = await  _repository.fetchPendingConfirmationOpportunities();
  ...
  fetchCompletedOpportunities(int status) async {
    try{
      emit(LoadingState());
      final response = await  _repository.fetchCompletedOpportunitiesByType(status);
      emit(Initialized<List<CompletedOpportunityDto>>(data: response));
    }catch(e){
      emit(ErrorState(e));
    }
  ...
  ReviewShiftsprams params = ReviewShiftsprams(pageNumber:  1, statusId: 0, pageSize: 10);

  StreamStateInitial<List<CompletedOpportunityData>?> opportunityStream = StreamStateInitial();
  ...
  Future<List<CompletedOpportunityData>> fetchOpportunityData(ReviewShiftsprams params) async {
